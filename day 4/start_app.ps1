...
$binLivekit = Join-Path (Split-Path -Parent $repoRoot) "bin\livekit-server.exe"
if (Test-Path $binLivekit) {
  Start-Process powershell -ArgumentList "-NoExit", "-Command", "& '$binLivekit' --dev"
