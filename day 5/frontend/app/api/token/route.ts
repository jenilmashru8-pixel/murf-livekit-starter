...
export async function POST(req: Request) {
  try {
    ...
    // Get persistent user_id from request (sent by frontend), or generate random as fallback
    const userId = body.user_id || `user_${Math.floor(Math.random() * 10_000)}`;
    ...
    // Generate participant token using persistent user_id
    const participantName = 'user';
    const participantIdentity = `voice_assistant_user_${userId}`;
    const roomName = `voice_assistant_room_${userId}`;
