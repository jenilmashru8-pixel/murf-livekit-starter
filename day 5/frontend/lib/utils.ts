...
import { getPersistentUserId } from '@/lib/user-storage';
...
/**
 * Get a token source for a regular LiveKit session with persistent user ID (Day 4)
 * Includes the user's persistent ID in the request so the backend can track them across calls
 * @param appConfig - The app configuration
 * @returns A token source with persistent user ID
 */
export function getTokenSourceWithPersistentUserId(appConfig: AppConfig) {
  return TokenSource.custom(async () => {
    // Get or generate persistent user ID from browser storage
    ...
    try {
      const res = await fetch('/api/token', {
        ...
        body: JSON.stringify({
          user_id: userId,  // Send persistent user ID to backend
          room_config: roomConfig,
        }),
      });
      ...
      console.error('Error fetching token with user ID:', error);
